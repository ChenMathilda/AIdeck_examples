...
void modelModel(unsigned int L1Memory, unsigned int L2Memory, unsigned int L3Memory, unsigned int L3Flash)
{
    ...
    CNN_Norm("S1_Op_input_1_formatter", 28, 28, 1, NNTOOL_KOP_BW);
    // generator for DEPTHWISE_CONV_2D_0_0_fusion
    CNN_ConvolutionPoolAct_SQ8("S2_Conv2d_32x1x3x3_MaxPool_2x2_Relu", 0, 4, 1, 1, 32, 28, 28,
        KOP_CONV, 3, 3, 1, 1, 2, 2, 0,
        KOP_MAXPOOL, 2, 2, 1, 1, 2, 2, 0,
        KOP_RELU);
    // generator for CONV_2D_0_2_fusion
    CNN_ConvolutionPoolAct_SQ8("S3_Conv2d_64x32x3x3_MaxPool_2x2_Relu", 0, 4, 1, 32, 64, 6, 6,
        KOP_CONV, 3, 3, 1, 1, 1, 1, 0,
        KOP_MAXPOOL, 2, 2, 1, 1, 2, 2, 0,
        KOP_RELU);
    // generator for FULLY_CONNECTED_0_4
    CNN_LinearAct_SQ8("S4_Linear_10x2x2x64", 0, 4, 1, 256, 10, KOP_LINEAR, KOP_NONE);
    // generator for SOFTMAX_0_5
    CNN_SoftMax_SQ8("S5_SoftMax", 0, 10, KOP_SOFTMAX);
...
#define GRAPH
    ...
    CreateGraph("modelCNN",
        /* Arguments either passed or globals */
            CArgs(18,
                ...
                // in: 0.125000 out: 0.000031 NORM: 12
    ...
    // Node S2_Conv2d_32x1x3x3_MaxPool_2x2_Relu inq -1.00<i8*0.00781250<0.99 weightsq chan<i8*chan<chan outq -2.54<i8*0.01987736<2.52 biasesq i32*chan
    ...
    // Node S3_Conv2d_64x32x3x3_MaxPool_2x2_Relu inq -2.54<i8*0.01987736<2.52 weightsq chan<i8*chan<chan outq -7.07<i8*0.05519716<7.01 biasesq i32*chan
    ...
    // Node FULLY_CONNECTED_0_4 inq -7.07<i8*0.05519716<7.01 weightsq chan<i8*chan<chan outq -14.62<i8*0.12500000<14.51
    AddNode("S4_Linear_10x2x2x64", Bindings(7, GNodeArg(GNA_IN, "S3_Output", 0), GNodeArg(GNA_IN, "S4_Weights", 0), GNodeArg(GNA_IN, "S4_Biases", 0), GNodeArg(GNA_OUT, "S4_Output", 0), GNodeArg(GNA_IN, "S4_Mul_scale", 0), GNodeArg(GNA_IN, "S4_Mul_shift", 0), GNodeArg(GNA_IN, "S4_Infos", 0)));
    // Node SOFTMAX_0_5 inq 3 outq 15
